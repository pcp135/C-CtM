...
def lin_comb_mat_vec_mult(M, v):
		assert(M.D[1] == v.D)
		answer = Vec(M.D[0],{})
		for (k,vector) in mat2coldict(M).items():
			answer = answer + (v[k]*vector)
		return answer
...
def lin_comb_vec_mat_mult(v, M):
		assert(v.D == M.D[0])
		answer = Vec(M.D[1],{})
		for (k,vector) in mat2rowdict(M).items():
			answer = answer + (v[k]*vector)
		return answer
...
def dot_product_mat_vec_mult(M, v):
		...
		answer = Vec(M.D[0],{})
		for (k,vector) in mat2rowdict(M).items():
			answer = answer + Vec(M.D[0],{k:v*vector})
		return answer
...
def dot_product_vec_mat_mult(v, M):
		...
		answer = Vec(M.D[1],{})
		for (k,vector) in mat2coldict(M).items():
			answer = answer + Vec(M.D[1],{k:v*vector})
		return answer
...
def Mv_mat_mat_mult(A, B):
		assert A.D[1] == B.D[0]
		answer = {}
		for (k,v) in mat2coldict(B).items():
			answer[k]=A*v
		return coldict2mat(answer)
...
def vM_mat_mat_mult(A, B):
		assert A.D[1] == B.D[0]
		answer = {}
		for (k,v) in mat2rowdict(A).items():
			answer[k]=v*B
		return rowdict2mat(answer)
...
def dot_prod_mat_mat_mult(A, B):
		assert A.D[1] == B.D[0]
		answer = Mat((A.D[0], B.D[1]),{})
		for (k1,v1) in mat2rowdict(A).items():
			for (k2,v2) in mat2coldict(B).items():
				answer[(k1,k2)]=v1*v2
		return answer
...
solving_systems_a_times_m = Mat(({0, 1}, {0, 1}), {(0, 1): 0, (1, 0): 0, (0, 0): 1.0, (1, 1): 1.0})
