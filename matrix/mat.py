...
def vector_matrix_mul(v, M):
		...
		assert M.D[0] == v.D
		return Vec(M.D[1], {k:sum([v[d]*M[(d,k)] for d in M.D[0]]) for k in M.D[1]})

def matrix_vector_mul(M, v):
		"Returns the product of matrix M and vector v"
		assert M.D[1] == v.D
		return Vec(M.D[0], {k:sum([v[d]*M[(k,d)] for d in M.D[1]]) for k in M.D[0]})

def matrix_matrix_mul(A, B):
		"Returns the product of A and B"
		assert A.D[1] == B.D[0]
		overlap = {(x,y,z) for (x,z) in A.f.keys() for (y1,y) in B.f.keys() if z==y1}
		key = defaultdict(list)
		for (x,y,z) in overlap:
			key[(x,y)]+=[z]
		return Mat((A.D[0],B.D[1]), {(x,y): sum([A[(x,c)]*B[(c,y)] for c in key[(x,y)]]) 
				for (x,y) in key.keys() })
